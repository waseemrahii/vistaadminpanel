...
import { createVendor,fetchVendors } from "../../../components/redux/vendorSlice";
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (createVendor.fulfilled.match(actionResult)) {
        ...
        event.target.reset(); // Reset form fields
        await dispatch(fetchVendors()); // Refresh vendor list
        navigate("/venderlist");
      } else {
        throw new Error(actionResult.error.message || "Failed to add vendor!");
      }
    } catch (error) {
      // Only display the error message, not the full object
      toast.error(error.message || "Failed to add vendor!");
